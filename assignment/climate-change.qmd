...
avg_temp <- temp_data |> 
  ...
  summarize(mean_temp = mean(No_Smoothing, na.rm = TRUE), .groups = "drop") |> 
...
To calculate a rolling average in a R, a simple command to use is the rollmean function. An example of how to use the rollmean function (produced by Stack Overflow) used the zoo package to run. I also saw that there is another way to do the rollmean function and that is by using the the dplyr or TTR packages. The TTR package offers a SMA() function to calculate the Simple Moving Average 4which works very similarly to the rollmean. It doesn't seem like you really need to modify the data once you have it, so this format kinda makes sense.
