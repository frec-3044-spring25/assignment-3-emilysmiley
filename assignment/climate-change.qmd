...
CO2 into the atmosphere. **Question 2:**
...
time when CO2 accumulation peaks before it can fully ramps up
...
growth and CO2 uptake take hold. \## Global Temperature Data
...
temp_data <- read.table(file_url,
                        sep = ",",
                        header = TRUE,
                        colnames(c("Year", "No_Smoothing", "Lowess(5)")),
                        comment.char = "#", 
                        skip = 4)
str(temp_data)

# Assuming the data has years and temperature anomalies (you can adjust column names based on your inspection)
# Rename the columns for clarity
#colnames(temp_data) <- c("Year", "No_Smoothing", "Lowess(5)")
...
```{r}
avg_temp <- temp_data |> 
  group_by(No_Smoothing) |> 
  summarize(mean_temp = mean(No_Smoothing, na.rm = TRUE))

ggplot(avg_temp, aes(x = Year, y = No_Smoothing)) +
  geom_point() +
  geom_line() +
...
To calculate a rolling average in a R, a simply command to use is the rollmean function. An example of how to use the rollmean function that was produced by Stack Overflow that used the zoo package to run. I also saw that there is another way to do the rollmean function and that is by using the the dplyr or TTR packages. For example, the TTR package offers a SMA() function to calculate the Simple Moving Average which works very similarly to the rollmean. It doesn't seem like you really need to modify the data once you have it, so this format kinda makes sense.
...
# Assume temp_data is the dataset with actual temperature anomaly data
# Example: Load your actual data (use read.table or read.csv, etc.)
# temp_data <- read.table("path_to_your_data.txt", header = TRUE)

# Create new columns with rolling averages using mutate()
temp_data <- temp_data |> 
  mutate(
    Rolling_5_Year = rollmean(Temperature_Anomaly, k = 5, fill = NA, align = "center"),
    Rolling_10_Year = rollmean(Temperature_Anomaly, k = 10, fill = NA, align = "center"),
    Rolling_20_Year = rollmean(Temperature_Anomaly, k = 20, fill = NA, align = "center")
  )

# Display the updated data frame with the new rolling average columns
temp_data
...
```{r}
# Plot the original temperature anomalies and the rolling averages
ggplot(temp_data, aes(x = Year)) +
  geom_line(aes(y = Temperature_Anomaly), color = "grey", size = 1) + # Original data
  geom_line(aes(y = Rolling_5_Year), color = "blue", size = 1.2) + # 5-year rolling average
  geom_line(aes(y = Rolling_10_Year), color = "red", size = 1.2) + # 10-year rolling average
  geom_line(aes(y = Rolling_20_Year), color = "green", size = 1.2) + # 20-year rolling average
